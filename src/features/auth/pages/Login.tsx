...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      initializeCrypto(password, data.user.id).catch(() => {
        /* silently ignore */
      })
      ...
      const errorMessage =
        error?.message || 'Error inesperado al iniciar sesión. Por favor intenta de nuevo.'
      showToast(errorMessage, 'error')
      setErrors({ general: errorMessage })
    } finally {
      setLoading(false)
        ...
        <div className="w-full max-w-sm">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between items-center pt-2 pb-2 text-xs md:text-sm text-white font-semibold tracking-wide px-2">
              <Link to="/forgot" className="hover:text-gray-300 transition-colors">
                ¿OLVIDASTE TU CONTRASEÑA?
              </Link>
              <Link to="/ayuda" className="hover:text-gray-300 transition-colors">
                AYUDA
              </Link>
            </div>
          </form>
