...
import {
  ...
  getDocumentsSharedByMeWith,
  uploadDocument,
  uploadDocumentEncrypted,
  deleteDocument,
  getFolders,
  createFolder,
  deleteFolder,
...
export default function Documentos() {
  ...
  const filterContent = () => {
    ...
    if (debouncedSearch) {
      ...
      filtDocs = filtDocs.filter(
        (doc) =>
          doc.title.toLowerCase().includes(search) || doc.notes?.toLowerCase().includes(search),
      )
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    const patientIdForUpload = currentFolder.id?.startsWith('shared-')
      ? currentFolder.id.replace('shared-', '')
      : null
    let successCount = 0
  ...
  const handleDeleteFolder = async (folderId: string) => {
    ...
    if (
      !confirm(
        '¿Estás seguro de eliminar esta carpeta? Se eliminarán también todos los documentos que contiene. Esta acción no se puede deshacer.',
      )
    )
      return
  ...
  const handleFolderClick = (id: string, name: string) => {
    ...
    setCurrentFolderInfo(
      folderMeta ? { avatarUrl: folderMeta.avatarUrl, subtitle: folderMeta.subtitle } : null,
    )
  }
  ...
  const handleRequestPatientAccess = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!user || !accessReqEmail.trim()) return
    setAccessReqLoading(true)
    try {
      const result = await requestAccessByIdentifier(
        user.id,
        accessReqEmail.trim(),
        accessReqReason.trim() || undefined,
      )
      if (result.ok) {
      ...
      showToast('Error inesperado al solicitar acceso', 'error')
    } finally {
  ...
  const handleSaveUrl = async (e: React.FormEvent) => {
    ...
    setSavingUrl(true)
    const patientIdForUrl = currentFolder.id?.startsWith('shared-')
      ? currentFolder.id.replace('shared-', '')
      : null
  ...
  const handleShareSubmit = async (
    ...
    const ids = shareTarget.type === 'document' ? [shareTarget.id] : shareTarget.documentIds || []
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length) {
      setUploadForm((prev) => ({
        ...
        title:
          prev.files.length === 0 && validFiles.length === 1
            ...
            {prescriptionsLoading ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Cuando tu médico te genere una receta aparecerá aquí
                </p>
              </div>
            ) : (
                ...
                {prescriptions.map((rx) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      {rx.folio && (
                        <span className="text-[10px] font-mono text-gray-400">#{rx.folio}</span>
                      )}
                    </div>
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      {new Date(rx.issued_at + 'T12:00:00').toLocaleDateString('es-MX', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                      })}
                  ...
                  !currentFolder.id
                    ...
                    : currentFolder.id.startsWith('shared-')
                      ? profile?.role === 'patient'
                        ...
                        : 'Buscar documentos del paciente…'
            ...
            <div className="flex items-center gap-1.5 text-sm text-gray-400 overflow-x-auto py-0.5 no-scrollbar">
              <button
                ...
                onDrop={(e) => {
                  ...
                  e.stopPropagation()
                  const payload =
                    e.dataTransfer.getData('application/healthpal-doc') ||
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-gray-900">
                  {urlMode ? 'Guardar Enlace' : 'Subir Documento'}
                </h2>
              </div>
              <button
                onClick={() => {
                  if (!uploading && !savingUrl) {
                    setUploadModalOpen(false)
                    setUrlMode(false)
                  }
                }}
            ...
            {currentFolder.id?.startsWith('shared-') && (
              <div className="mx-5 mt-4 flex items-start gap-2 bg-primary/10 text-primary rounded-xl px-3.5 py-2.5 text-xs">
                ...
                <span>
                  Este documento se guardará en tu biblioteca y se compartirá automáticamente con{' '}
              ...
              <form onSubmit={handleSaveUrl} className="p-5 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Liga de radiología, portal de laboratorio u otro enlace médico.
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setUrlForm((prev) => ({ ...prev, category: e.target.value as DocCategory }))
                    }
              ...
              <form onSubmit={handleUpload} className="p-5 space-y-4">
                  ...
                  >
                    ...
                    {uploadForm.files.length > 0 ? (
                        ...
                        <div className="flex items-center gap-3 text-left">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-500">
                              {(uploadForm.files[0].size / 1024 / 1024).toFixed(2)} MB · Clic para
                              agregar más
                            </p>
                          </div>
                          <button
                        ...
                        <div className="text-left w-full" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-xs font-semibold text-gray-700">
                              {uploadForm.files.length} archivos seleccionados
                            </p>
                            <button
                              type="button"
                              onClick={() => document.getElementById('file-upload')?.click()}
                      ...
                      <>
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                          Puedes seleccionar varios a la vez · PDF, JPG, PNG, MP4 y más · Máx. 10 MB
                          c/u
                        </p>
                      </>
                    )}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setUploadForm((prev) => ({ ...prev, document_date: e.target.value }))
                    }
                    max={new Date().toISOString().split('T')[0]}
                    className="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary text-sm transition-all"
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    La fecha real del documento, no la de subida (ej. fecha del análisis o receta)
                  </p>
                </div>
                  ...
                  >
                    {uploading ? (
                      ...
                      <>
                        <Upload size={16} />
                        {uploadForm.files.length > 1
                          ? `Subir ${uploadForm.files.length} archivos`
                          : 'Subir Documento'}
                      </>
                    )}
                  </button>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={handleRequestPatientAccess} className="p-5 space-y-4">
              <p className="text-sm text-gray-500">
                El paciente recibirá la solicitud y podrá autorizar compartir su expediente desde su
                configuración.
              </p>
              <div>
