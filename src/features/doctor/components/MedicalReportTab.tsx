...
import {
  ...
  fetchTemplatePdf,
  MedicalReport,
} from '@/shared/lib/queries/medicalReports'
import { getClinicalHistory } from '@/shared/lib/queries/clinicalHistory'
import { getInformeId } from '@/shared/lib/queries/insurance'
...
// Build a map of field-name → value to pre-fill based on patient data + clinical history + insurance
...
function buildFillMap(
  ...
  const sexLabel =
    patient?.sex === 'male' ? 'Masculino' : patient?.sex === 'female' ? 'Femenino' : ''
  ...
  return {
    ...
    __alcohol: nph.alcohol?.present
      ? `Sí — ${nph.alcohol.frequency_per_week ?? '?'} veces/sem`
      : 'No',
    ...
    __diabetes: ph.diabetes?.present
      ? `Sí${ph.diabetes.details ? ` — ${ph.diabetes.details}` : ''}`
      : '',
    __hipertension: ph.hypertension?.present
      ? `Sí${ph.hypertension.details ? ` — ${ph.hypertension.details}` : ''}`
      : '',
    ...
    // Insurance fields
    __no_poliza: insurance?.policy_number ?? '',
    __no_asegurado: insurance?.member_id ?? '',
    __titular: insurance?.holder_name ?? '',
    __parentesco_titular: insurance?.holder_relationship ?? '',
    __no_grupo: insurance?.group_number ?? '',
    __vigencia_desde: fmtDate(insurance?.valid_from),
    __vigencia_hasta: fmtDate(insurance?.valid_until),
    __tipo_cobertura: insurance?.coverage_type ?? '',
    __tel_siniestros: insurance?.phone_claims ?? '',
    __tel_emergencias_seguro: insurance?.phone_emergency ?? '',
    __aseguradora: insurance?.provider_name ?? '',
    // Emergency contact from medProfile
    __contacto_emergencia: medProfile?.emergency_contact_name ?? '',
    __tel_emergencia: medProfile?.emergency_contact_phone ?? '',
  }
}
...
function resolveFieldValue(fieldName: string, fillMap: Record<string, string>): string | null {
  if (matches(fieldName, 'apellido paterno', 'primer apellido', '1er apellido'))
    return fillMap.__apellido_paterno
  if (matches(fieldName, 'apellido materno', 'segundo apellido', '2do apellido'))
    return fillMap.__apellido_materno
  if (
    matches(
      fieldName,
      'nombre(s)',
  ...
  if (matches(fieldName, 'tipo de sangre', 'grupo sanguineo', 'tipo sangre', 'blood'))
    return fillMap.__tipo_sangre
  ...
  if (matches(fieldName, 'ultima menstruacion', 'fecha menstruacion', 'fur', 'fum'))
    return fillMap.__ultima_menstruacion
  ...
  if (
    matches(fieldName, 'fecha del informe', 'fecha informe', 'fecha elaboracion', 'fecha reporte')
  )
    return fillMap.__fecha_informe
  // Insurance fields
  if (matches(fieldName, 'poliza', 'póliza', 'no. poliza', 'numero poliza', 'num poliza', 'policy'))
    return fillMap.__no_poliza
  if (
    matches(
      fieldName,
      'no. asegurado',
    ...
    return fillMap.__no_asegurado
  if (matches(fieldName, 'titular', 'contratante', 'holder')) return fillMap.__titular
  if (matches(fieldName, 'parentesco', 'relacion con titular')) return fillMap.__parentesco_titular
  if (matches(fieldName, 'grupo', 'no. grupo', 'group')) return fillMap.__no_grupo
  if (matches(fieldName, 'vigencia desde', 'inicio vigencia', 'fecha inicio', 'valid from'))
    return fillMap.__vigencia_desde
  if (
    matches(fieldName, 'vigencia hasta', 'fin vigencia', 'vencimiento', 'valid until', 'valid to')
  )
    return fillMap.__vigencia_hasta
  if (matches(fieldName, 'tipo cobertura', 'cobertura', 'plan', 'coverage'))
    return fillMap.__tipo_cobertura
  if (matches(fieldName, 'telefono siniestro', 'tel siniestro', 'tel reclamacion', 'claims'))
    return fillMap.__tel_siniestros
  if (matches(fieldName, 'aseguradora', 'compania seguros', 'compañia', 'insurance', 'insurer'))
    return fillMap.__aseguradora
  // Emergency contact
  if (matches(fieldName, 'contacto emergencia', 'nombre emergencia', 'emergency contact'))
    return fillMap.__contacto_emergencia
  if (matches(fieldName, 'telefono emergencia', 'tel emergencia', 'emergency phone'))
    return fillMap.__tel_emergencia
  return null
}
...
export default function MedicalReportTab({
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([getMedicalReports(patientId, doctorId), getClinicalHistory(patientId)])
      .then(([reps, hist]) => {
        setReports(reps)
  ...
  const openPdf = useCallback(
    async (aseguradora: (typeof ASEGURADORAS)[0], existingReport?: MedicalReport) => {
      setPdfLoading(true)
      ...
      try {
        ...
        // Find matching insurance for this aseguradora
        const matchedInsurance =
          patientInsurances.find((ins) => getInformeId(ins.provider_name) === aseguradora.id) ??
          (patientInsurances.length === 1 ? patientInsurances[0] : null)
        const fillMap = buildFillMap(patient, medProfile, clinicalHistory, matchedInsurance)
        const { blobUrl, pdfBytes, fieldsFound, fieldsFilled } = await loadAndFillPdf(
          aseguradora.file,
          fillMap,
        )
    ...
    [patient, medProfile, clinicalHistory, patientId, doctorId, patientInsurances],
  )
  ...
  const markCompleted = async () => {
    ...
    try {
      ...
      setReports((prev) =>
        prev.map((r) => (r.id === activeReport.id ? { ...r, status: 'completed' } : r)),
      )
      ...
      <div className="flex flex-col h-full" style={{ minHeight: 'calc(100vh - 220px)' }}>
        ...
        <div className="flex items-center gap-3 px-1 pb-3 flex-shrink-0 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-sm font-black text-gray-900 break-words">
              {activeAseguradora.label}
            </h2>
          </div>
        ...
        {/* PDF iframe */}
        <div
          className="flex-1 rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-gray-50 relative"
          style={{ minHeight: 600 }}
        >
          {pdfLoading && (
        ...
        <p className="text-[10px] text-gray-400 mt-2 text-center">
          Los campos pre-llenados vienen del perfil del paciente · Completa los restantes
          directamente en el documento · Descarga o imprime al terminar
        </p>
      </div>
    )
  }
        ...
        <div className="space-y-2">
          ...
          {reports.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:border-[#33C7BE]/30 transition-all"
              >
                <div
                  className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${r.status === 'completed' ? 'bg-green-50' : 'bg-amber-50'}`}
                >
                  {r.status === 'completed' ? (
