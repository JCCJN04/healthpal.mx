...
function Cie10Row({
  ...
  return (
    <div className="flex items-start gap-2 group">
      {/* Badge tipo */}
      <span
        className={`mt-2 text-[9px] font-bold w-14 shrink-0 uppercase tracking-wide ${index === 0 ? 'text-violet-600' : 'text-gray-400'}`}
      >
        ...
        <div ref={wrapRef} className="relative">
          <div className="flex items-center border border-dashed border-gray-300 bg-gray-50 rounded-lg overflow-hidden focus-within:border-violet-300 focus-within:bg-white focus-within:border-solid transition-all">
            ...
            <input
              ...
              placeholder={
                codigo ? 'Cambiar código CIE-10…' : 'Buscar código o diagnóstico en español…'
              }
          ...
          {open && results.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
              {results.map((r) => (
                ...
                >
                  <span className="text-[10px] font-mono font-bold text-violet-700 shrink-0 w-14">
                    {r.code}
                  </span>
...
function SoapSection({
  ...
  return (
    <div
      className={`rounded-2xl border ${hasContent ? 'border-gray-200 bg-white' : 'border-dashed border-gray-200 bg-gray-50/50'} transition-all`}
    >
      <div
        className={`flex items-center justify-between px-4 py-2.5 border-b ${hasContent ? 'border-gray-100' : 'border-transparent'}`}
      >
...
function NotaEvolucionReadonly({
  nota,
  onAddenda,
}: {
  nota: NotaEvolucion
  onAddenda: (nota: NotaEvolucion) => void
}) {
  ...
  const hasVitals =
    nota.ta_sistolica || nota.frecuencia_cardiaca || nota.temperatura || nota.peso_kg
              ...
              {nota.ta_sistolica && (
                <div className="text-center col-span-2 sm:col-span-1">
                  ...
                  <p className="text-sm font-bold text-gray-800">
                    {nota.ta_sistolica}/{nota.ta_diastolica}
                  </p>
            ...
            <div className="space-y-1.5">
              ...
              {(nota.notas_evolucion_diagnosticos ?? []).map((d, i) => (
                ...
                >
                  ...
                  <span className="text-xs font-mono font-bold text-violet-700 bg-violet-100 px-2 py-0.5 rounded-lg shrink-0">
                    {d.cie10_codigo}
                  </span>
                  {d.cie10_descripcion && (
                    <span className="text-xs text-gray-600 truncate">{d.cie10_descripcion}</span>
                  )}
                </div>
              ))}
            </div>
...
export default function NotaEvolucionForm({ appointmentId, patientId, existing, onSaved }: Props) {
  ...
  const [pronostico, setPronostico] = useState(existing?.pronostico ?? '')
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-gray-400 ml-1">{completedCount}/6</span>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white">
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100">
          <div
            className={`w-1 h-5 rounded-full ${completion.vitals ? 'bg-primary' : 'bg-gray-200'}`}
          />
                  ...
                  <button
                    ...
                    onMouseDown={() => {
                      if (
                        exploracion.trim() &&
                        !window.confirm(
                          'Esto reemplazará el texto que ya escribiste en Exploración Física. ¿Continuar?',
                        )
                      )
                        return
      ...
      <div
        className={`rounded-2xl border ${completion.a ? 'border-gray-200 bg-white' : 'border-dashed border-gray-200 bg-gray-50/50'} transition-all`}
      >
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100">
          <div
            className={`w-1 h-5 rounded-full ${completion.a ? 'bg-amber-400' : 'bg-gray-200'}`}
          />
      ...
      <SoapSection
        ...
        value={pronostico}
        onChange={setPronostico}
        placeholder="Evolución esperada, factores pronósticos, tiempo estimado de recuperación..."
        rows={2}
      />
