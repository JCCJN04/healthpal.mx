...
export default function AgendarCitaModal({
  ...
  async function handleDocReq(e: React.FormEvent) {
    ...
    setDocReqError('')
    try {
      const { data, error: reqErr } = await createDocumentRequest(
        patientEmail,
        docReqType.trim(),
        docReqDesc.trim() || undefined,
      )
      if (reqErr || !data) {
        logger.error('AgendarCitaModal:docReq', reqErr)
        setDocReqError('No se pudo generar la solicitud de documento.')
        return
      }
      setDocReqLink(`${window.location.origin}/solicitud/${data.token}`)
    } catch (err) {
      logger.error('AgendarCitaModal:docReq:catch', err)
      setDocReqError('Error de conexión al generar la solicitud.')
    } finally {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        logger.error('AgendarCitaModal:availability', err)
      })
      .finally(() => {
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {step === 1 && (
            <>
              ...
              <button
                onClick={() => setStep(2)}
                disabled={!date || !time || loadingAvailability}
            ...
            <>
              ...
              <div className="flex items-center gap-2 px-3 py-2 bg-teal-50 rounded-lg border border-teal-100 text-sm">
                ...
                <span className="font-medium text-gray-700">
                  ...
                  {time} · {duration} min · {MODE_TO_LABEL[mode]}
                </span>
              </div>
              ...
              <div>
                ...
                {reason.length > 0 && reason.trim().length < 5 && (
                  <p className="text-[11px] text-red-400 mt-1">
                    Mínimo 5 caracteres para continuar.
                  </p>
                )}
              </div>
              ...
              <div className="rounded-xl overflow-hidden">
                ...
                >
                  ...
                  <span className="text-sm font-semibold">
                    Solicitar documentos antes de la cita
                  </span>
                    ...
                    {docReqLink ? (
                      <div className="space-y-2">
                        <p className="text-xs text-green-700 font-semibold">
                          ✓ Enlace generado. Compártelo con el paciente:
                        </p>
                      ...
                      <form onSubmit={handleDocReq} className="space-y-2">
                        ...
                        {docReqError && <p className="text-[11px] text-red-500">{docReqError}</p>}
                      </form>
                    )}
                  </div>
                )}
