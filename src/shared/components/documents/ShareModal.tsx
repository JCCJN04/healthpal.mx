...
export function ShareModal({
  ...
  useEffect(() => {
    ...
    listDocumentShares(documentId)
      .then((data) => {
        if (!cancelled) setCurrentShares((data as CurrentShare[]) || [])
      })
      .catch((err) => {
        console.error('ShareModal:listDocumentShares', err)
      })
      .finally(() => {
              ...
              <div className="min-w-0">
                ...
                {isFolder && folderDocCount !== undefined && (
                  <p className="text-[10px] text-primary font-medium">
                    {folderDocCount} documento{folderDocCount !== 1 ? 's' : ''} incluido
                    {folderDocCount !== 1 ? 's' : ''}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {loadingShares ? (
                  ...
                  <p className="text-xs text-gray-400 py-1">
                    Nadie tiene acceso a este documento todavía
                  </p>
                ) : (
