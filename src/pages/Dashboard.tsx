          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {patientSnapshot.slice(0, 5).map((p: { patient_id: string; last_interaction: string; full_name?: string | null }) => {
                ...
                return (
                  <div key={p.patient_id} className="flex items-center justify-between py-2 border-b last:border-b-0 border-gray-100">
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{p.full_name || 'Paciente sin nombre'}</p>
                      <p className="text-xs text-gray-500">ID: {p.patient_id.slice(0, 6)}…</p>
                    </div>
...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [upcomingData, markedDaysData, documentsData, sharedDocuments, notificationsData, snapshot, doctorPatients] = await Promise.all([
