...
async function handleInitiate(
  ...
  await supabaseAdmin.from('account_merge_tokens').delete().eq('uuid_b', uuidB).is('used_at', null)

  // Generate token: store only the hash, send the raw value
  const rawToken = generateSecureToken()
  const tokenHash = await sha256Hex(rawToken)

  const { error: insertErr } = await supabaseAdmin.from('account_merge_tokens').insert({
    uuid_a: uuidA,
    uuid_b: uuidB,
    phone,
  ...
  if (insertErr) {
    ...
    return new Response(JSON.stringify({ error: 'No se pudo iniciar la fusión' }), {
      status: 500,
      headers: { ...cors, 'Content-Type': 'application/json' },
    })
  }
  ...
  try {
    ...
    await supabaseAdmin.from('account_merge_tokens').delete().eq('token_hash', tokenHash)
    console.error('sendConfirmationEmail:', (emailErr as Error).message)
    return new Response(JSON.stringify({ error: 'No se pudo enviar el correo de confirmación' }), {
      status: 502,
      headers: { ...cors, 'Content-Type': 'application/json' },
    })
  }
...
async function handleConfirm(
  ...
  if (record.used_at) {
    return new Response(JSON.stringify({ error: 'Este token ya fue utilizado' }), {
      status: 400,
      headers: { ...cors, 'Content-Type': 'application/json' },
    })
  }
  ...
  if (new Date(record.expires_at) < new Date()) {
    return new Response(
      JSON.stringify({
        error: 'El enlace de confirmación ha expirado. Inicia el proceso de nuevo.',
      }),
  ...
  if (markErr) {
    ...
    return new Response(JSON.stringify({ error: 'Error interno — por favor intenta de nuevo' }), {
      status: 500,
      headers: { ...cors, 'Content-Type': 'application/json' },
    })
  }
  ...
  const {
    data: { user: userB },
  } = await supabaseAdmin.auth.admin.getUserById(uuidB)
