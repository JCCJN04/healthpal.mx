...
const ALLOWED_ORIGINS = new Set(['null', 'https://www.healthpal.mx', 'http://localhost:3000'])
...
async function checkRateLimit(
  ...
  const now = new Date()
  const resetAt = new Date(Math.ceil(now.getTime() / 3_600_000) * 3_600_000).toISOString()

  const { data, error } = await supabase
    .from('rate_limit_buckets')
    .upsert({ key, count: 1, reset_at: resetAt }, { onConflict: 'key', ignoreDuplicates: false })
    .select('count')
    .single()
...
async function callLLM(
  ...
  if (isOpenRouterKey && isOpenAiEndpoint) {
    console.error(
      'ai-proxy: OpenRouter key detected but OPENAI_BASE_URL points to OpenAI. Set OPENAI_BASE_URL=https://openrouter.ai/api/v1',
    )
...
async function handlePatientSummary(raw: PatientSummaryInput): Promise<string | null> {
  const lines: string[] = []
  ...
  // Clinical
  if (raw.chronicConditions)
    lines.push(`Condiciones crónicas: ${sanitizeInput(raw.chronicConditions)}`)
  ...
  if (raw.hospitalizations)
    lines.push(`Hospitalizaciones: ${sanitizeInput(raw.hospitalizations, 200)}`)

  // Psychiatric / developmental
  if (raw.psychiatricDiagnoses)
    lines.push(`Antecedentes psiquiátricos: ${sanitizeInput(raw.psychiatricDiagnoses, 300)}`)
  if (raw.psychiatricMeds)
    lines.push(`Medicación psiquiátrica: ${sanitizeInput(raw.psychiatricMeds, 200)}`)
  if (raw.developmentalNotes)
    lines.push(`Antecedentes de desarrollo: ${sanitizeInput(raw.developmentalNotes, 200)}`)
  ...
  // Family history
  if (raw.familyHistory)
    lines.push(`Antecedentes heredofamiliares: ${sanitizeInput(raw.familyHistory, 300)}`)

  // Appointments & notes
  if (raw.totalAppointments != null)
    lines.push(`Consultas totales con usted: ${Number(raw.totalAppointments)}`)
  if (raw.lastAppointmentDate)
    lines.push(`Última consulta: ${sanitizeInput(raw.lastAppointmentDate, 30)}`)
  ...
  if (raw.documentCount != null)
    lines.push(`Documentos en expediente: ${Number(raw.documentCount)}`)

  // Patient observations from clinical history
  if (raw.patientObservations)
    lines.push(`Observaciones del paciente: ${sanitizeInput(raw.patientObservations, 300)}`)

  // Recent notes content
  if (raw.recentNotesSummary)
    lines.push(`Contenido notas recientes: ${sanitizeInput(raw.recentNotesSummary, 500)}`)
  ...
  const userPrompt = `Genera el resumen clínico para el médico tratante:\n${lines.join('\n')}`
...
async function handlePatientChat(raw: PatientChatInput): Promise<string | null> {
  ...
  if (patient.chronicConditions)
    patientBlock.push(`Crónicas: ${sanitizeInput(patient.chronicConditions)}`)
  ...
  const history = (Array.isArray(raw.history) ? raw.history : []).slice(-10).map((t) => ({
