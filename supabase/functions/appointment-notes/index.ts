  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!appointment_id || !body?.trim()) {
        return new Response(JSON.stringify({ error: 'appointment_id y body son requeridos' }), {
          status: 400,
          headers: { ...cors, 'Content-Type': 'application/json' },
        })
      }

      // Validate JWT server-side — never trust locally-decoded claims
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase.from('appointment_notes').delete().eq('id', noteId)
