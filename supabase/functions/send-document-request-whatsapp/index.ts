...
serve(async (req) => {
  ...
  try {
    ...
    console.log('payload', {
      ...
      doctor_name: typeof doctor_name,
    ...
    if (!UUID_RE.test(String(document_request_id))) {
      ...
      return new Response(JSON.stringify({ error: 'Invalid document_request_id' }), {
        status: 400,
        headers: { ...cors, 'Content-Type': 'application/json' },
      })
    }
    if (!UUID_RE.test(String(doctor_id))) {
      ...
      return new Response(JSON.stringify({ error: 'Invalid doctor_id' }), {
        status: 400,
        headers: { ...cors, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (String(doctor_id) !== user.id) {
      return new Response(JSON.stringify({ error: 'Forbidden: doctor_id mismatch' }), {
        status: 403,
        headers: { ...cors, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (!PHONE_RE.test(cleanPhone)) {
      ...
      return new Response(JSON.stringify({ error: 'Invalid patient_phone format' }), {
        status: 400,
        headers: { ...cors, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (docReqErr || !docReq) {
      return new Response(JSON.stringify({ error: 'Document request not found' }), {
        status: 404,
        headers: { ...cors, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { error: sessionErr } = await supabase.from('whatsapp_sessions').insert({
    ...
    formBody.set(
      'ContentVariables',
      JSON.stringify({ '1': cleanDoctorName, '2': cleanDocumentType }),
    )
