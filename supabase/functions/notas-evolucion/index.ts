...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === 'POST' && isAddenda) {
      ...
      if (!nota_id || !contenido?.trim())
        return new Response(JSON.stringify({ error: 'nota_id y contenido son requeridos' }), {
          status: 400,
          headers: { ...cors, 'Content-Type': 'application/json' },
        })
    ...
    if (req.method === 'GET') {
      ...
      if (!appointmentId && !patientId)
        return new Response(JSON.stringify({ error: 'appointment_id o patient_id requerido' }), {
          status: 400,
          headers: { ...cors, 'Content-Type': 'application/json' },
        })
      ...
      const decrypted = await Promise.all(
        (data || []).map(async (nota: any) => ({
          ...
          exploracion_fisica: await decryptField(
            nota.exploracion_enc,
            nota.exploracion_nonce,
            aesKey,
          ),
          diagnostico: await decryptField(nota.diagnostico_enc, nota.diagnostico_nonce, aesKey),
          pronostico: await decryptField(nota.pronostico_enc, nota.pronostico_nonce, aesKey),
          plan_terapeutico: await decryptField(nota.plan_enc, nota.plan_nonce, aesKey),
          notas_evolucion_diagnosticos: (nota.notas_evolucion_diagnosticos || []).sort(
    ...
    if (req.method === 'POST') {
        ...
        .upsert(
          {
            ...
            doctor_id: userId,
            patient_id,
            ta_sistolica: ta_sistolica || null,
            ta_diastolica: ta_diastolica || null,
            ...
            motivo_enc: mEnc?.enc || null,
            motivo_nonce: mEnc?.nonce || null,
            exploracion_enc: eEnc?.enc || null,
            exploracion_nonce: eEnc?.nonce || null,
            diagnostico_enc: dEnc?.enc || null,
            diagnostico_nonce: dEnc?.nonce || null,
            pronostico_enc: prEnc?.enc || null,
            pronostico_nonce: prEnc?.nonce || null,
            plan_enc: pEnc?.enc || null,
            plan_nonce: pEnc?.nonce || null,
            enc_kid: keyId,
            enc_ver: keyVer,
          },
      ...
      if (notaError)
        return new Response(JSON.stringify({ error: notaError.message }), {
          status: 400,
          headers: { ...cors, 'Content-Type': 'application/json' },
        })
      ...
      if (diagRows.length > 0) {
        ...
        if (diagError)
          return new Response(JSON.stringify({ error: diagError.message }), {
            status: 400,
            headers: { ...cors, 'Content-Type': 'application/json' },
          })
      }
