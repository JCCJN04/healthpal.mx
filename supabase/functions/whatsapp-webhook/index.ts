...
async function checkRateLimit(
  ...
  if (error) {
    console.warn('rate_limit rpc error (blocking):', error.message)
    return false // fail closed — prevent abuse during outages
  }
...
async function sendWhatsAppReply(
  ...
  if ('contentSid' in bodyOrTemplate) {
    ...
    if (bodyOrTemplate.contentVariables) {
      formBody.set('ContentVariables', JSON.stringify(bodyOrTemplate.contentVariables))
    }
  } else {
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const sigValid = await validateTwilioSignature(twilioAuthToken, webhookUrl, params, twilioSig)
    ...
    if (!filename) {
      ...
      const looksLikeFilename =
        bodyText.length > 0 && bodyText.length <= 200 && /\.[a-zA-Z0-9]{2,5}$/.test(bodyText)
    ...
    const saveFile = async (
      ...
      if (recipients.length > 0) {
        try {
          ...
          console.warn(
            'Encryption failed, falling back to plain upload:',
            (encErr as Error).message,
          )
          docKey = null
        }
      }
      ...
      if (isEncrypted && docKey && docIv) {
        for (const recipient of recipients) {
          try {
            ...
            if (keyError)
              console.warn(
                `document_keys insert failed for user ${recipient.userId}:`,
                keyError.message,
              )
          } catch (wrapErr) {
            console.warn(
              `Key wrap failed for user ${recipient.userId}:`,
              (wrapErr as Error).message,
            )
          }
        }
      }

      const { error: archivoError } = await supabase.from('archivos_recibidos').insert({
    ...
    if (session?.document_request_id) {
      ...
      if (!patientId) {
        ...
        for (const ph of phoneCandidates) {
          ...
          if (stripped.startsWith('521') && stripped.length === 13) {
            ...
            stripped.startsWith('52') &&
            !stripped.startsWith('521') &&
            stripped.length === 12
          ) {
      ...
      await supabase.from('whatsapp_sessions').update({ status: 'fulfilled' }).eq('id', session.id)

      if (await shouldSendReply()) {
        await sendWhatsAppReply(twilioAccountSid, twilioAuthToken, twilioFrom, fromRaw, {
    ...
    if (profile) {
      ...
      const { data: authUserId, error: rpcError } = await supabase.rpc('auth_user_id_by_phone', {
      ...
      if (authUserId && !rpcError) {
        ...
        if (existingProfile) {
          ...
          console.log('Scenario 2: auth user exists, phone not linked:', authUserId)
          const patientPubKey2 = await getPatientPublicKey(authUserId)
          const recipients2 = patientPubKey2
            ? [{ userId: authUserId, publicKey: patientPubKey2 }]
            : []
          await saveFile(
            authUserId,
            null,
